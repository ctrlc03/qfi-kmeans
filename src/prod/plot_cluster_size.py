...
def read_data(): 
    ...
    with open(f"./src/prod/gitcoin/cosine/outputs/data_{filename}_k_{k}-means.json") as sizes:
        _sizesCosine = json.load(sizes)

    with open(f"./src/prod/gitcoin/eucledian/outputs/data_{filename}_k_{k}-means.json") as sizes:
...
def plot_by_size_of_clusters(k, sizes, cosine):
    ...
    plt.savefig(
        f'./src/prod/gitcoin/{output}/plots/{sys.argv[2]}_k_{k}_sizes.png', dpi=300
    )
