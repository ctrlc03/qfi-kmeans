...
def read_data(): 
    k = sys.argv[1]
    with open(f"./tests/data/output_k_means_plus_plus_k_{k}.json") as in_file:
...
def plot_by_size_of_clusters(clusters, sizes):
    ...
    # save
    plt.savefig('./tests/plots/random_indexes/plot_k_means_plus_plus_k_{k}.png'.format(k=clusters), dpi=300)
...
def plot_by_user_coefficient(coefficients, voters, k):
    ...
    # save
    plt.savefig('./tests/plots/random_indexes/plot_k_means_plus_plus_coefficients_k_{k}.png'.format(k=clusters), dpi=300)
